...
$save = 5;		 # the number of saved parameters from optimize.pl
...
#	@gp = (@gpar0,@gpar1,@gpar2,@gpar3,@gpar4) = ([$gpa,$gpb],[$gpa,$gpb],[$gpa,$gpb],[$gpa,$gpb],[$gpa,$gpb]);
	@gp = (@gpar0,@gpar1,@gpar2,@gpar3,@gpar4) = ([0.03,0.1],[0.02,0.2],[0.02,0.1],[0.01,0.1],[0.01,0.2]);
}
# debugging
#$k = 0;
#$f_seqhl = "SEQ_globins";
...
for ($k=0; $k<$save; ++$k)
{
	if ($cgporngp == 1)
	{
		...
		@command_line = "loopp.exe -x -seq $f_seqhl -xyz $f_xyzhl -i 2 -fps -rsc $beta[$k] -fd0 $d0[$k] -g $gp[$k] $gp[$k] -pick $query -nprn $n2prnt -best $n_best -tr 50 $local\n";
		print @command_line;
		system(@command_line);
	}
	if ($cgporngp == 0)
	{
		...
		@command_line = "loopp.exe -x -seq $f_seqhl -xyz $f_xyzhl -i 2 -fps -rsc $beta[$k] -fd0 $d0[$k] -g 0.3 0.3 -gpar $gp[$k][0] $gp[$k][1] -pick $query -nprn $n2prnt -best $n_best -tr 50 $local\n";
		print @command_line;
		system(@command_line);
	}
...
#	prnt_array(\@x_rmsd,"x_rmsd");
	...
	# get % seq identity of best matched proteins to query
...
#	prnt_array(\@$seqid,"seqid");
	...
	@command_line2 = "loopp.exe -s $f_seqhl -seq $f_seqhl -xyz $f_xyzhl -i 2 -pick $query -nprn $n2prnt -best $n_best $local\n";
	print @command_line2;
...
#	print "max energy = $max\n";
	...
	if ($cgporngp == 0)
	{
		print DATA "run: beta = $beta, d0 = $d0, gpara = $$gp[$k][0], gparb = $$gp[$k][1]\n";
	}
	if ($cgporngp == 1)
	{
		print DATA "run: beta = $beta, d0 = $d0, cgp = $gp[$k]\n";
	}
	print DATA "query = $query and database = $f_seqhl and $f_xyzhl\n";
	print DATA "command line = @command_line\n";
	print DATA "xnames\t\t\tx_ener\t\t\tx_rmsd\t\t\tfamily\t\t\tnon-family\n";
	for ($i=0; $i<$n; ++$i)
	{
		print DATA "$xnames[$i]\t\t\t\t$x_ener[$i]\t\t\t\t$x_rmsd[$i]\t\t\t\t$family[$i]\t\t\t\tnxfmnames[$i]\n";
	}
	print DATA "\n";
