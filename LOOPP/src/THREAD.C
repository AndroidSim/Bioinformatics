		...
		k_mps *var_mps[], k_mps *tail_mps[])
{
 ...
 while(read_seq(F->query,&Lseq,Q_name,query_Seq,&i_prot,&op->n_type,Alph,Adr))
   {
     i_prot=0;
	 ...
	 read_seq(F->seq,&Lstru,T_name,nat_Seq,&i_prot,&op->n_type,Alph,Adr);
		 ...
		 int *aGaps, int *Stru_row)
{
 ...
 while(read_contMap(F,op,IJcont,&Lstru,Tmpl_name,r12ij,r6ij,rij))
   {   
     ...
     read_seq(F->seq,&Lstru,Tmpl_name,nat_Seq,&i_prot,&op->n_type,Alph,Adr);
	      ...
	      k_mps *var_mps[],k_mps *tail_mps[])
{
 ...
 while(read_contMap(F,op,IJcont,&Lstru,T_name,r12ij,r6ij,rij))
   {    
    ...
    while(read_seq(F->seq,&Lseq,R_name,seq,&i_prot,&op->n_type,Alph,Adr) 
	  && (i_prot<=op->end))
      {
		  ...
		  int *Alph,int *Adr,k_mps *var_mps[],k_mps *tail_mps[])
{
 ...
 /* get structure of R_name as represented in F->type */
 /* to get Lseq */
 read_seq(F->seq,&Lseq,R_name,seq,&i_prot,&op->n_type,Alph,Adr); 
 if(!read_contType(F,op,contR,R_name,contRvdw)) read_err(R_name);
 ...
 while(read_seq(F->seq,&Lseq,T_name,seq,&i_prot,&op->n_type,Alph,Adr))
   {   
