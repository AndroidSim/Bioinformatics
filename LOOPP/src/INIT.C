...
/* if(op->fngrps) F->fngrps=fopen(F->f_fps,"w"); */
 ...
 if (op->fngrps)
 {
	 ...
	 if (F->fngrps==NULL)
	 {
		 ...
		 if (F->fngrps==NULL)
		 {
			 printf("cannot open fingerprint file already present for read or writing\n");
			 exit(1);
		 }
	 }
 }
...
void set_default_opt(a_file *F, k_opt *op, k_info *Info)
{
 ...
 /* options and defaults settings for fingerprints and profiles are stored in 
	the fingerprnts structure pointed to by fps */
 fps=&fps_data;
 fps->norm_d0=(double)0.5;
 fps->fps_potn=0;
 ...
 for(i=0;i<argc;++i)
   {
	 ...
	 /* some command line arguments for fingerprints */
	 fps=&fps_data;
	 if(strcmp(argv[i],"-fd0")==0)
	 { 
		 if(plus_1) fps->norm_d0=atof(argv[i+1]);
	 }
	 if (strcmp(argv[i],"-fpp")==0) /* fpp = fingerprint potential */
	 {
		 if (plus_1)
		 {
			 if (strcmp(argv[i+1],"eij") /*atoi(argv[i+1])*/==0)
			 {
				 fps->fps_potn=0;
			 }
			 if (strcmp(argv[i+1],"evdw")/*atoi(argv[i+1])*/==0)
			 {
				 fps->fps_potn=1;
			 }
			 if (strcmp(argv[i+1],"ein")==0)
			 {
				 fps->fps_potn=2;
			 }
			 if (strcmp(argv[i+1],"einm")==0)
			 {
				 fps->fps_potn=3;
			 }
		 }
