...
void get_optimal_score(Dynamic *DP,opshuns *op,sqs *seq,int rs_or_zs)
{
	...
	/* for global alignment (op->local==0) optimal score is bottom-right corner of dynamic
	   matrix */
	if (op->local==0)
	{
	...
	/* for local alignment (op->local==1) optimal score is the maximum score throughout the
	   entire dynamic matrix. */
	/* the best local alignment is one that has a high (good) score and the longest alignment
	   length */
	if (op->local==1)
	{
		for (i=0; i<n_rows; ++i)
